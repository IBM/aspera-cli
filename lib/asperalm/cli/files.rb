...
require 'optparse'
require 'pp'
require 'aspera/browser_interaction'
...
class CliFiles
  ...
  def go(argv,defaults)
    begin
      ...
      command=AsperaOptParser.get_next_arg_from_list(argv,'command',[ :send, :recv, :events, :jwt, :set_client_key, :faspexgw, :files_res, :admin ])
      ...
      when :set_client_key
        the_client_id=AsperaOptParser.get_next_arg_value(argv,'client_id')
        the_private_key=AsperaOptParser.get_next_arg_value(argv,'private_key')
        api_files_admin=Rest.new(@logger,files_api_base_url,{:oauth=>api_files_oauth,:scope=>@@SCOPE_FILES_ADMIN})
      ...
      when :admin
        api_files_admin=Rest.new(@logger,files_api_base_url,{:oauth=>api_files_oauth,:scope=>@@SCOPE_FILES_ADMIN})
        resource=AsperaOptParser.get_next_arg_from_list(argv,'resource',[:clients,:contacts,:dropboxes,:nodes,:operations,:packages,:saml_configurations])
        #:messages:organizations:url_tokens,:usage_reports:workspaces
        operation=AsperaOptParser.get_next_arg_from_list(argv,'operation',[:list])
        case operation
        when :list
