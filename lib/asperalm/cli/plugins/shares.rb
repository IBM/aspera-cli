  ...
  module Cli
    module Plugins
      class Shares < BasicAuthPlugin
        ...
        def execute_action
          api_shares=Rest.new(Main.tool.options.get_option_mandatory(:url)+'/node_api',{:basic_auth=>{:user=>Main.tool.options.get_option_mandatory(:username), :password=>Main.tool.options.get_option_mandatory(:password)}})
