      ...
      class Shares < BasicAuthPlugin
        ...
        def execute_action
          ...
          when :admin
            ...
            command=self.options.get_next_argument('command',[:user,:share])
            case command
            when :user
              ...
              when :id
                ...
                command=self.options.get_next_argument('command',[:app_authorizations,:authorize_share])
                case command
            ...
            when :share
              command=self.options.get_next_argument('command',[:list,:name])
              all_shares=api_shares_admin.read('data/shares')[:data]
              case command
              when :list
