...
module Asperalm
  module Cli
    class Main
      ...
      #--------------------------------
      def go(argv)
        $DEFAULT_CONFIG_FILE=File.join(Dir.home,'.aspera/ascli/config.yaml')
        defaults={
          :logtype => :stdout,
          :loglevel => :warn,
          :config_name => 'default'
        ...
        begin
          ...
          app_name=OptParser.get_next_arg_from_list(argv,'application',@@COMANDS)
          ...
          require 'asperalm/cli/'+app_name.to_s
          application=Object::const_get('Asperalm::Cli::'+app_name.to_s.capitalize).new(@logger)
