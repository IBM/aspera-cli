...
module Aspera
  ...
  class CommandLineBuilder
    ...
    CLI_OPTION_TYPE_SWITCH = %w[opt_without_arg opt_with_arg].freeze
    CLI_OPTION_TYPES = %w[special ignore envvar].concat(CLI_OPTION_TYPE_SWITCH).freeze
    OPTIONS_KEYS = %w[description type x-type default enum agents required cli ts deprecation].freeze
    CLI_KEYS = %w[opt-type switch convert variable].freeze
    ...
    class << self
      ...
      def read_description(source_path, suffix=nil)
        ...
        YAML.load_file("#{source_path[0..-4]}#{suffix}.yaml").each do |name, options|
          ...
          Aspera.assert(options['cli'].key?('opt-type')){'Missing key: cli.type'}
          Aspera.assert_values(options['cli']['opt-type'], CLI_OPTION_TYPES){"Unsupported processing type for #{name}"}
          # by default : optional
          options[:mandatory] ||= false
          options['description'] ||= ''
          options['description'] = "DEPRECATED: #{options[:deprecation]}\n#{options['description']}" if options.key?(:deprecation)
          # replace "back solidus" HTML entity with its text value
          options['description'] = options['description'].gsub('&bsol;', '\\')
          cli = options['cli']
          ...
          options['type'] ||= options['cli']['opt-type'].eql?('opt_without_arg') ? 'boolean' : 'string'
          # add default switch name if not present
          if !cli.key?('switch') && cli.key?('opt-type') && CLI_OPTION_TYPE_SWITCH.include?(cli['opt-type'])
    ...
    def process_param(name, read: false)
      ...
      processing_type = read ? 'get_value' : options['cli']['opt-type']
      ...
      expected_classes = case options['type']
      ...
      expected_classes.push(options['x-type']) if options.key?('x-type')
      # check that value is of expected type
      raise Transfer::Error, "#{name} is : #{parameter_value.class} (#{parameter_value}), shall be #{options['type']}, " \
