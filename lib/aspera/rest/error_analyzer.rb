...
module Aspera
  module Rest
    ...
    #   Returns the singleton instance of ErrorAnalyzer
    #   @return [ErrorAnalyzer] the singleton instance
    class ErrorAnalyzer
      include Singleton
      ...
      def initialize
        ...
        add_handler('Type Generic') do |type, context|
          if !context[:response].code.start_with?('2')
            ...
            ErrorAnalyzer.add_error(context, type, "#{context[:request]['host']} #{context[:response].code} #{context[:response].message}")
          end
        end
      end

      # Use this method to analyze a EST result and raise an exception
      # Analyzes REST call response and raises a CallError exception
      ...
      def raise_on_error(req, data, http)
        ...
        raise CallError, context unless context[:messages].empty?
      end
        ...
        add_handler(name) do |type, context|
          if context[:data].is_a?(Hash) && (!context[:response].code.start_with?('2') || always)
            ...
            if error_struct.is_a?(Hash) && error_struct[path.last].is_a?(String)
              ErrorAnalyzer.add_error(context, type, error_struct[path.last])
