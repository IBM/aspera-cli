  ...
  module Transfer
    ...
    class SpecDoc
      class << self
        ...
        def man_table(formatter, include_option: false)
          ...
          Spec::SCHEMA['properties'].filter_map do |name, properties|
            ...
            AGENT_LIST.each do |agent_info|
              columns[agent_info.last] = formatter.tick(properties['x-agents'].nil? || properties['x-agents'].include?(agent_info.first.to_s))
            end
            columns[col_local] = formatter.tick(true) if properties['x-cli-option']
