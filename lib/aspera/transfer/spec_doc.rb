  ...
  module Transfer
    ...
    class SpecDoc
      class << self
        ...
        # @param formatter      [Cli::Formatter] Formatter to use, methods: markdown_to_terminal, tick, check_row
        ...
        def man_table(formatter, include_option: false, agent_columns: true, schema: Spec::SCHEMA)
          ...
          schema['properties'].each do |name, info|
            ...
            columns[:description] =
              info['description']
                .gsub(Markdown::FORMATS){formatter.markdown_to_terminal(Regexp.last_match)}
            ...
            columns[:description].push("Allowed values: #{info['enum'].map{ |v| formatter.markdown_to_terminal("`#{v}`")}.join(', ')}") if info.key?('enum')
            if include_option
              envvar_prefix = ''
              cli_option =
              ...
              columns[:description].push("(#{'special:' if info['x-cli-special']}#{envvar_prefix}#{formatter.markdown_to_terminal("`#{cli_option}`")})#{short}") if cli_option
            end
