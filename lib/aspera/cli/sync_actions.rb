      ...
      def execute_sync_action(&block)
        ...
        when :start
          ...
          if simple_session_args.empty?
            ...
            raise "provide zero or 3 arguments: #{SIMPLE_ARGUMENTS_SYNC.keys.join(',')}" unless simple_session_args.keys.sort == SIMPLE_ARGUMENTS_SYNC.keys.sort
            async_params = options.get_option(
              :sync_info,
              mandatory: false,
              default: {'sessions' => [{'name' => File.basename(simple_session_args['local_dir'])}]})
            raise "sync_info shall be a Hash with key 'sessions' with Array of Hash: #{async_params}" unless async_params.is_a?(Hash) && async_params['sessions']&.is_a?(Array) && async_params['sessions'].first.is_a?(Hash)
