...
Command line arguments formatted as `<%=ph :name%>` in examples represent user-provided values, not literal values.
...
```text
Use only the attached <%=cmd%> manual (it matches the version I use).
...
If you use [Chocolatey](https://chocolatey.org/), <%=tool%> can be installed with the `aspera-cli` package: it installs Ruby (if not already present) and the `<%=gemspec.name%>` gem.
...
Use a version of JRuby compatible with a Ruby version supported by <%=tool%>.
...
<%=tool%> detects most Aspera transfer products in standard locations and use the first one found by default.
...
rm -fr temp_folder
tar zcvf <%=gemspec.name%>-<%=build_version%>-gems.tgz <%=gemspec.name%>-<%=build_version%>-gems
```
...
```shell
--user root --env <%=opt_env :home%>=/home/cliuser/.aspera/<%=cmd%> --volume $HOME/.aspera/<%=cmd%>:/home/cliuser/.aspera/<%=cmd%>
```
...
For example, files transferred with <%=tool%> through folder `/xferfiles` (right-hand side) would be available on the host in `$HOME/xferdir`.
...
A convenience sample script is also provided: download the script [`d<%=cmd%>`](<%=link_repo('build/container/d'+cmd)%>) from [the GitHub repository](https://raw.githubusercontent.com/IBM/aspera-cli/main/build/container/d<%=cmd%>).

Some environment variables can be set for this script to adapt its behavior:

| env var        | Description                        | Default                  | Example                  |
|----------------|------------------------------------|--------------------------|--------------------------|
| `<%=opt_env :home%>` | Configuration folder (persistency) | `$HOME/.aspera/<%=cmd%>` | `$HOME/.<%=cmd%>_config` |
...
The wrapping script maps the folder `$<%=opt_env :home%>` on the host to `/home/cliuser/.aspera/<%=cmd%>` in the container (the configuration folder expected in the container).
...
```shell
curl -o <%=cmd%> https://raw.githubusercontent.com/IBM/aspera-cli/main/build/container/d<%=cmd%>
chmod a+x <%=cmd%>
export xferdir=$HOME/xferdir
mkdir -p $xferdir
chmod -R 777 $xferdir
...
To change the configuration, tell `ascp` to use another file using `ascp` option `-f`, for example, by locating it on the host folder `$HOME/.aspera/<%=cmd%>` mapped to the container folder `/home/cliuser/.aspera/<%=cmd%>`:
...
To update trusted root certificates for <%=tool%>, update the system's root certificate store (see the locations displayed above).

An up-to-date version of the certificate bundle can also be retrieved with:

```shell
<%=cmd%> config echo @uri:https://curl.se/ca/cacert.pem --format=text
```

To download that certificate store:

```shell
<%=cmd%> config echo @uri:https://curl.se/ca/cacert.pem --format=text --out.file=/tmp/cacert.pem
```
...
> [!NOTE]
> Not all features of <%=tool%> are documented here: explore commands on the command line, using `-h`.
...
Such subcommands appear after the resource identifier, for example, in `<%=cmd%> aoc admin node do <%=ph :node_id%> browse /`, `browse` is a subcommand of `node`.
...
> Use `END` as usual to stop collection when further positional arguments must follow: `<%=cmd%> aoc tier_restrictions --query=@: a=b END other_arg`.
...
Environment variables starting with prefix <%=opt_env ''%> are taken as option values, for example, `<%=opt_env :option_name%>` is for `--option-name`.
...
```batchfile
set <%=opt_env :home%>=C:\Users\Kenji\.aspera\<%=cmd%>
...
When OAuth is used (AoC, Faspex 5), <%=tool%> keeps a cache of generated bearer tokens in folder `persist_store` located in the configuration folder by default.
...
```shell
<%=cmd%> config preset set <%=ph :aoc_org%> private_key @val:@file:"$HOME/.aspera/<%=cmd%>/<%=ph :pkey_name%>"
```
...
```yaml
<%=ph :aoc_org%>:
  private_key: "@file:/Users/laurent/.aspera/<%=cmd%>/<%=ph :pkey_name%>"
```
...
```text
<%=cmd%> config preset set <%=ph :preset_name%> <%=ph :parameter_name%> <%=ph :parameter_value%>
```
...
- If option `--preset=<%=ph :name%>` is specified (or `-P<%=ph :name%>`), this reads the [Option Preset](#option-preset) specified from the configuration file by name.
- If option `--preset=<%=ph :extended_value_hash%>` is specified, it is used as option values (`Hash` of option/value pairs).
...
```shell
<%=cmd%> -N --url=_url_here_ --password=<%=ph :password%> --username=_name_here_ node --show-config
```
...
<%=cmd%> config preset set shares06 password <%=ph :password%>
```
...
```shell
KEY_PAIR_PATH=~/.aspera/<%=cmd%>/<%=ph :pkey_name%>
```
...
By default, <%=tool%> assumes that a graphical environment is available on Windows; on other systems, it relies on the presence of the `DISPLAY` environment variable.
...
```shell
export http_proxy=http://<%=ph :username%>:<%=ph :password%>@proxy.example.com:3128
```

Option `http_proxy` does the same (set env var) but on command line:

```shell
<%=cmd%> --http-proxy=http://<%=ph :username%>:<%=ph :password%>@host:port ...
```
...
By default, <%=tool%> gets notifications from `ascp` on its management port.
...
The name of the file shall be: `send_<%=ph :PID%>`, where `<%=ph :PID%>` is the process ID of the running <%=tool%>.
...
This command is not available on clients: instead, edit the file `aspera.conf` (its location is shown by `<%=cmd%> config ascp info --fields=aspera_conf`) and modify the sections `default` and `trunks` like this for a global 100 Mbps virtual link (capacity is in kbps):
...
```shell
<%=cmd%> config ascp info --fields=aspera_conf
```
...
```shell
--transfer=@json:'{"agent":"node","url":"https://...","username":"_user_here_","password":"<%=ph :password%>"}'
```
...
As such, it can be modified with option: `--ts=@json:'{"destination_root":"<%=ph :path%>"}'`.
...
This section is not specific to <%=tool%>: it describes `ascp` behavior.
...
For this reason, it is recommended to set `create_dir` to `true` for consistent behavior between single and multiple item transfers; this is the default in <%=tool%>.
...
```shell
--ts=@json:'{"content_protection":"decrypt","content_protection_password":"<%=ph :password%>"}'
```
...
- Scheduled execution: run <%=tool%> commands periodically.

- Daemon/service mode: run <%=tool%> continuously as a server.
...
# Forward arguments to <%=cmd%> and ensure it does not exceed 30 min execution
exec timeout 30m <%=cmd%> "${@}"
```
...
Define a name for the job, for example: `<%=cmd%>_job` as `<%=ph :name%>` below.
...
1. Create the service
    ...
    ```ini
    [Unit]
    Description=Run <%=cmd%> preview scan
...
1. Create the timer (same base name)
    ...
    `/etc/systemd/system/<%=ph :name%>.timer`

    ```ini
    [Unit]
    Description=Hourly execution of <%=ph :name%>
...
1. Enable the timer
    ...
    sudo systemctl enable --now <%=ph :name%>.timer
    ```

Check status:

```shell
systemctl status <%=ph :name%>.timer
```

```shell
systemctl list-timers | grep <%=ph :name%>
```

Run service once to test:

```shell
sudo systemctl start <%=ph :name%>.service
```
...
2-59 * * * *  /home/xfer/bin/<%=cmd%>_tool preview trevents --logger=syslog --out.level=error
EOF
```
...
In this case, it is recommended to run <%=tool%> as a system service.

On Linux this is typically done using [`systemd`](https://systemd.io/).

For example, below `<%=ph :name%>` is `<%=cmd%>_svc`.

A [wrapping script](#creating-a-wrapping-script), again, is convenient: `/usr/local/bin/start_<%=ph :name%>.sh`:
...
[Service]
ExecStart=/usr/local/bin/start_<%=ph :name%>.sh
Restart=on-failure
RestartSec=15
User=xfer
...
```shell
sudo systemctl daemon-reload
sudo systemctl enable --now <%=ph :name%>.service
systemctl status <%=ph :name%>.service
journalctl -u <%=ph :name%>.service
```

#### Preventing Concurrent Execution

When running scheduled jobs, it may be necessary to ensure that only one instance of <%=tool%> runs at a time.
...
```shell
<%=ph :adapter%>:///<%=ph :sub_file_path%>?<%=ph :arg1%>=<%=ph :val1%>&...
```
...
```shell
cp /opt/aspera/lib/pvcl/libpvcl_cloud.so $(<%=cmd%> config ascp info --fields=root)
```
...
For all sizes, a suffix can be added (case-insensitive) to the size: k, m, g, t, p, e (values are powers of 2, for example, 1M is 2<%=pow 20%>, that is, 1 mebibyte, not megabyte).
...
The way arguments are parsed and provided to <%=tool%> depends on the operating system and shell.
...
Ruby receives the list of command line arguments from the shell and gives it to <%=tool%>.
...
Some values provided to <%=tool%> (options, **Command Parameters**) are expected to be [Extended Values](#extended-value-syntax), that is, not a simple `String`, but a composite structure (`Hash`, `Array`).
...
Enclose the variable in double quotes so that it results in a single argument for <%=tool%> even if its value contains spaces: `bash` splits the result of an unquoted variable expansion into several words (`zsh` does not, by default).
...
> [!IMPORTANT]
> In the following example, replace `https://<%=ph :org_name%>.ibmaspera.com` with your actual organization URL.
> `(User)` represents the user menu on the top right of the page.

```console
$ <%=cmd%> config wizard
Using: Aspera on Cloud at https://<%=ph :org_name%>.ibmaspera.com
Path to private RSA key (leave empty to generate):
option: key_path>
Using existing key:
/home/john/.aspera/<%=cmd%>/<%=ph :pkey_name%>.pem
Please Log in as user john@example.com at: https://<%=ph :org_name%>.ibmaspera.com
...
Preparing preset: aoc<%=ph :org_name%>ibmaspera_com_john_example_com
...
- **Direct Plugin Targeting**: To limit detection to the `aoc` plugin and skip discovery, provide both the URL and plugin name:
  ...
  ```shell
  <%=cmd%> config wizard <%=ph :org_name%> aoc
  ```
...
- Open a web browser, log in to your instance: for example, `https://<%=ph :org_name%>.ibmaspera.com/`
...
option: url> https://<%=ph :org_name%>.ibmaspera.com/
option: client_id> <%=ph :client_id%>
option: client_secret> <%=ph :client_secret%>
updated: <%=ph :aoc_org%>
```

> [!NOTE]
> In above example, replace `https://<%=ph :org_name%>.ibmaspera.com` with your actual AoC URL.

(This can also be done in one line using the command `config preset update <%=ph :aoc_org%> --url=...`)

Define this [Option Preset](#option-preset) as default configuration for the `aoc` plugin:

```shell
<%=cmd%> config preset set default aoc <%=ph :aoc_org%>
```
...
- Graphically
  ...
  - Open a web browser, log in to your instance: `https://<%=ph :org_name%>.ibmaspera.com/`
    (Use your actual AoC instance URL)
  - Go to Apps &rarr; Admin &rarr; Organization &rarr; Integrations
...
Open the previously generated public key located here: `$HOME/.aspera/<%=cmd%>/<%=ph :pkey_name%>.pub`
...
```shell
<%=cmd%> aoc user profile modify @: public_key=@file:~/.aspera/<%=cmd%>/<%=ph :pkey_name%>.pub
```
...
> [!TIP]
> The `<%=cmd%> aoc user profile show` command can be used to verify modifications.
...
```shell
<%=cmd%> config preset update <%=ph :aoc_org%> --auth=jwt --private-key=@val:@file:~/.aspera/<%=cmd%>/<%=ph :pkey_name%> --username=someuser@example.com
```
...
```shell
<%=cmd%> aoc admin node bearer_token <%=ph :node_id%>
```
...
It allows actions (create, update, delete) on **resources**: users, groups, nodes, workspaces, and so on, with the `admin <%=ph :resource_type%>` commands.

#### Listing resources

The command `aoc admin <%=ph :type%> list` lists all entities of given type.
...
<%=cmd%> aoc user settings show <%=ph :id%>
<%=cmd%> aoc user settings modify <%=ph :id%> @json:'{"value":"..."}'
```
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> v3 transfer list --query.q='*' --query.max=5
```
...
```shell
<%=cmd%> aoc admin workspace_membership create @json:'{"workspace_id":<%=ph :workspace_id%>,"member_type":"group","member_id":<%=ph :group_id%>}'
```
...
```shell
<%=cmd%> config wizard https://sedemo.ibmaspera.com aoc aoc_sedemo --username=someuser@example.com
```
...
```shell
<%=cmd%> aoc admin node create @: url=https://aspera.example.com access_key=<%=ph :access_key_id%> name=testnode1
```
...
1. Find the application ID in the workspace:
   ...
   ```shell
   <%=cmd%> aoc admin workspace show <%=ph :workspace_id%> --query.include_aspera_apps=true --fields=packages_app.id
   ```
...
```shell
<%=cmd%> aoc files node_info /src_folder --format=json --out.level=data | <%=cmd%> aoc packages send @json:'{"name":"test","recipients":["someuser@example.com"]}' 10M.dat --transfer=@json:@stdin: --transfer.agent=node
```
...
```shell
<%=cmd%> aoc packages recv <%=ph :package_id%> [<%=ph :file%> ...]
```
...
- `{"fld":["name","id"]}`
  - The subfolder name is `<%=ph :name%>.<%=ph :id%>`.
...
```shell
<%=cmd%> aoc files download <%=ph :single_file_path%>
```
...
- Admin Shared Folders: Administrators share folders from any node in a workspace using `aoc admin node do <%=ph :node_id%> permission`.
...
```shell
<%=cmd%> aoc files permission --workspace=<%=ph :workspace_name%> <%=ph :path_to_folder%> ...
```
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission --workspace=<%=ph :workspace_name%> <%=ph :path_to_folder%>
```
...
```shell
<%=cmd%> aoc files short_link <%=ph :path_to_folder%> private create
...
To get the node ID of the default node for workspace `<%=ph :workspace_name%>`, use the command:

```shell
<%=cmd%> aoc admin workspace show %name:'<%=ph :workspace_name%>' --fields=node_id
```
...
```shell
<%=cmd%> aoc admin workspace list --select.name="<%=ph :workspace_name%>" --fields=node_id
```
...
```shell
<%=cmd%> aoc admin workspace list --query.q="<%=ph :workspace_name%>" --fields=node_id
```
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> mkdir <%=ph :folder_path%>
```

Create the shared folder in workspace `<%=ph :workspace_name%>` (set `with` to empty string, or do not specify it).
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission <%=ph :folder_path%> create @json:'{"with":"","as":"folder_for_users"}' --workspace="<%=ph :workspace_name%>"
```
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission <%=ph :folder_path%> create @json:'{"with":"john@example.com","as":"folder_for_one_user"}' --workspace="<%=ph :workspace_name%>"
```

```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission <%=ph :folder_path%> create @json:'{"with":"group 1","as":"folder_for_a_group"}' --workspace="<%=ph :workspace_name%>"
```

```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission <%=ph :folder_path%> create @json:'{"with":"<%=ph :workspace_name%>","as":"folder_for_all_workspace"}' --workspace="<%=ph :workspace_name%>"
```
...
```shell
<%=cmd%> aoc admin workspace shared_folder %name:'<%=ph :workspace_name%>' list
```
...
```shell
<%=cmd%> aoc admin workspace shared_folder %name:'<%=ph :workspace_name%>' member 198 list
```
...
```shell
<%=cmd%> aoc admin node do 8666 permission /project1 list --query=@json:'{"tag":"aspera.files.workspace.id=<%=ph :workspace_id%>"}'
```
...
```shell
<%=cmd%> aoc admin workspace list --select=@json:'{"name":"<%=ph :workspace_name%>"}' --fields=id
```
...
```shell
<%=cmd%> aoc admin node do <%=ph :node_id%> permission %id: list --query=@json:'{"access_type":"user","access_id":"ASPERA_ACCESS_KEY_ADMIN_WS_<%=ph :workspace_id%>"}'
```
...
- Check that access works and locate the source folder `<%=ph :source_folder%>` and the file `<%=ph :source_file%>` in it, for example, using command `files browse`
...
```shell
<%=cmd%> -Porg1 aoc files node_info <%=ph :source_folder%> --format=json --out.level=data | <%=cmd%> -Porg2 aoc files upload <%=ph :source_file%> --to-folder=<%=ph :dest_folder%> --transfer=@json:@stdin: --transfer.agent=node
```
...
- `files node_info <%=ph :source_folder%>` generates transfer information for the source folder: Node API URL, credentials and root file ID, suitable for the next command
...
- `files upload <%=ph :source_file%> --to-folder=<%=ph :dest_folder%>` uploads the file `<%=ph :source_file%>` (located in `<%=ph :source_folder%>` of `org1`) to `<%=ph :dest_folder%>` in `org2`
...
```shell
<%=cmd%> aoc admin node do %name:'<%=ph :node_name%>' find ...
```
...
```shell
<%=cmd%> config preset update <%=ph :preset_name%> --ibm-api-key=<%=ph :secret%>_api_key_here
```

```shell
<%=cmd%> config preset set default ats <%=ph :preset_name%>
```
...
```shell
<%=cmd%> config preset update <%=ph :preset_name%> --instance=aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee
```
...
```shell
<%=cmd%> config preset update <%=ph :preset_name%> --ats-key=ats_XXXXXXXXXXXXXXXXXXXXXXXX --ats-secret=YYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYYY
```
...
```shell
<%=cmd%> server --url=ssh://_server_address_here_:33001 --username=_user_here_ --ssh-keys=_private_key_path_here_ --passphrase=_passphrase_here_
```
...
The command `find <%=ph :folder%> [filter_expr]` is available for **gen4/access key**, under `access_keys do self`.

The argument `<%=ph :folder%>` is mandatory and is the root from which the search is performed.
...
`<%=cmd%> node stream create @json:<%=ph :value%>`
with the following [**transfer-spec**](#transfer-specification):

```json
{"direction":"send","source":"udp://233.3.3.4:3000?loopback=1&ttl=2","destination":"udp://233.3.3.3:3001/","remote_host":"localhost","remote_user":"stream","remote_password":"<%=ph :password%>"}
```
...
```shell
<%=cmd%> node download /share/sourcefile --to-folder=/destination_folder --preset=aws_shod --transfer=@preset:azure_ats --transfer.agent=node
```
...
Using <%=tool%>, an access key can be created using the `access_keys create` command on the node (using main node credentials) or on ATS.
...
```shell
<%=cmd%> faspex5 packages send --url='https://faspex5.example.com/?context=_some_long_string_here_'
```
...
| `<%=ph :name%>`            | Name of shared inbox or workgroup.<%=br%>If option `group_type` is `shared_inboxes`: name of a shared inbox (default).<%=br%>If `group_type` is `workgroups`: name of workgroup. |
...
When `Strings` are provided, <%=tool%> automatically looks up the contact across all recipient types.
...
To list the content of a received package, use command `faspex5 packages browse <%=ph :package_id%>`.
...
```shell
<%=cmd%> faspex5 admin shared_inboxes invite_external_collaborator '%name:the shared inbox' @: email_address=john@example.com
```
...
> [!TIP]
> The shared folder can be identified by its numerical `id` or by name using the [percent selector](#percent-selector): `%<%=ph :field%>:<%=ph :value%>`. For example: `--shared-folder=3` or `--shared-folder=%name:partages`.
...
To send a password reset link to a user, use command `faspex5 admin accounts reset_password <%=ph :account_id%>`.
...
```shell
curl -H "Authorization: $(<%=cmd%> faspex5 bearer_token)" https://faspex5.example.com/aspera/faspex/api/v5/api_endpoint_here
```
...
```json
{
  "apikey": "<%=ph :api_key%>",
  "cos_hmac_keys": {
    "access_key_id": "<%=ph :access_key%>",
    "secret_access_key": "<%=ph :secret%>"
  },
  "endpoints": "https://control.cloud-object-storage.cloud.ibm.com/v2/endpoints",
  "iam_apikey_description": "<%=ph :description%>",
  "iam_apikey_name": "<%=ph :name%>",
...
```shell
<%=cmd%> -P<%=ph :preview_preset_name%> node browse /
```
...
Nevertheless, <%=tool%> may or may not have direct file system access to the access key storage root.

<%=tool%> provides 2 ways to read and write files with the option: `root_url`:

- Using direct access to the local file system.
- Using Aspera to transfer files between the storage and the <%=tool%> server.
...
Moreover, <%=tool%> supports sync with applications requiring token-based authorization.
...
- If `<%=ph :sync_info%>` is not provided, a default configuration is automatically generated in the `conf` format (details in the next section).
...
<%=tool%> does **not** have a standalone `hotfolder` command.
...
<%=tool%> may be used as a simple hot folder engine.
...
- <%=tool%> configuration capabilities and server-side knowledge
...
Once <%=tool%> command line arguments are defined, run the command using the OS native scheduler, for example, every minute or every 5 minutes.
...
<%=cmd%> config preset set smtp_google password <%=ph :password%>
```
...
```json
{"remote_host":"demo.asperasoft.com","remote_user":"asperaweb","ssh_port":33001,"remote_password":"<%=ph :password%>","direction":"receive","destination_root":"./test.dir","paths":[{"source":"/aspera-test-dir-tiny/200KB.1"}],"resume_level":"none"}
```
...
```text
asession
{"remote_host":"demo.asperasoft.com","ssh_port":33001,"remote_user":"asperaweb","remote_password":"<%=ph :password%>","direction":"receive","destination_root":".","keepalive":true,"resume_level":"none"}
...
To activate, set env var `<%=opt_env :enable_ecdsha2%>` to `true`.
...
By default, <%=tool%> creates a configuration file `~/.aspera/sdk/aspera.conf` like this:
