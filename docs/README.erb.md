...
```bash
MYVAR="Hello World"
<%=cmd%> conf echo @json:'{"title":"'$MYVAR'"}'
<%=cmd%> conf echo @json:{\"title\":\"$MYVAR\"}
```
...
```bash
<%=cmd%> conf echo @json:{\"title\":\"Test\ \\\"\ \'\ \&\ \\\\\"}
<%=cmd%> conf echo @json:'{"title":"Test \" '\'' & \\"}'
<%=cmd%> conf echo @ruby:"{'title'=>%q{Test \" ' & \\\\}}"
```
...
```bash
<%=cmd%> conf echo @ruby:"{'title'=>File.read('title.txt')}"
<%=cmd%> conf echo @ruby:"{'title'=>ENV['MYTITLE']}"
```
