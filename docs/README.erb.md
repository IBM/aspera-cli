...
Note that it gets its value after shell command line parsing and <%=tool%> extended value parsing.
...
In <%=tool%>, all transfer parameters, including file list, are kept in <%=trspec%> so that execution of a transfer is independent of the transfer agent (direct, connect, node, transfer sdk...).
...
Examples:
  ...
  * Providing file list directly to ascp using the pseudo <%=trspec%> parameter `EX_file_list` (works only with `direct` agent)

  ```javascript
  ... --sources=@ts --ts=@json:'{"paths":[],"EX_file_list":"filelist.txt"}'
  ```

  * Provide file list with `ascp` arguments using the pseudo <%=trspec%> parameter `EX_ascp_args` (works only with `direct` agent)
