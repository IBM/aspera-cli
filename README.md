...
OPTIONS:
        ...
        --bearer-key=VALUE           RSA private key PEM value for bearer token generation
        --token-info=VALUE           Information for bearer token (Hash)
...
- Create a private key (organization key) that will be used to sign bearer tokens:
  ...
  > `ascli` kindly extracts the public key with extension `.pub`
...
node async show ALL
node basic_token
node bearer --bearer-key=my_private_key --token-info=@json:'{"user_id":"1"}'
node browse / -r
