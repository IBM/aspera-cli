...
For example they are used in Aspera on Cloud, but any application integrating Aspera can use it.
...
- We assume here that an access key was created, and default configuration is set to use it.
...
- The corresponding public key shall be placed in the access key to be able to check the signature of the bearer token:
  ...
  > **Note:** Either the public or private key can be provided, and only the public key is used. Alternatively, use the following equivalent command, as `ascli` kindly extracts the public key with extension `.pub`:
...
- Now, let's assume we do not have access key credentials, and we want to do (limited) operations using the bearer token:
  ...
  ```bash
  ascli node --username=@ruby:nil --password="Bearer $(cat bearer.txt)" --root-id=$my_folder_id access_key do self br /
  ```

  > **Note:** We reset here the username to nil, but using the access key also would work but is not necessary. Only the url and the bearer token used as password are required.

### Node sample commands

```bash
node --username=@ruby:nil --password="Bearer $bearer_666" --root-id=$root_id access_key do self br /
