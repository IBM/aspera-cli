...
* `ascli` : ("Amelia") The Multi Layer IBM Aspera tool
...
```
$ ascli server browse / --url=ssh://demo.asperasoft.com:33001 --username=asperaweb --password=demoaspera
:............:...........:......:........:...........................:.......................:
...
```
$ ascli config id demoserver update --url=ssh://demo.asperasoft.com:33001 --username=asperaweb --password=demoaspera
updated: demoserver
$ ascli config id default set server demoserver
...
```
$ ascli -Pdemoserver server upload ~/mysample.file secondfile
```

This is the same as:

```
$ ascli -Pdemoserver server upload --sources=@args ~/mysample.file secondfile
```
...
Note that this is different from the "ascp" command line. The paradigm used by `ascli` is: all transfer parameters are kept in [_transfer-spec_](#transferspec) so that execution of a transfer is independent of the transfer agent. It is envisioned that, one day, ascp will accept a [_transfer-spec_](#transferspec) directly.
...
ascli --version)
ascli --version` TOOLNAME=EXENAME erb README.erb.md > README.md
ascli --warnings --config-file=ASCLI_CONFIG_FILE
...
ascli -N aspera files browse / --link=CF_AOC_PUBLINK_FOLDER
ascli -N aspera files upload --to-folder=/ CF_SAMPLE_FILEPATH --link=CF_AOC_PUBLINK_FOLDER
ascli -N aspera org --link=CF_AOC_PUBLINK_RECV_PACKAGE
ascli -N aspera packages send --value=@json:'{"name":"'"CURRENT_DATE"'"}' CF_SAMPLE_FILEPATH --link=CF_AOC_PUBLINK_SEND_DROPBOX
ascli -N aspera packages send --value=@json:'{"name":"'"CURRENT_DATE"'"}' CF_SAMPLE_FILEPATH --link=CF_AOC_PUBLINK_SEND_USER
ascli -N server --url=my_url_here --username=my_username_here --password=my_password_here --format=nagios nagios transfer --to-folder=CF_HSTS_FOLDER_UPLOAD
ascli -N server --url=my_url_here --username=my_username_here --ssh-keys=CF_HSTS_TEST_KEY --format=nagios nagios app_services
ascli -N server --url=my_url_here --username=my_username_here --ssh-keys=CF_HSTS_TEST_KEY ctl all:status
ascli -N server --url=my_url_here --username=my_username_here --ssh-keys=CF_HSTS_TEST_KEY nodeadmin -- -l
ascli -Pserver_eudemo_key server br /
ascli -Ptest_preview node upload ~/'Documents/Samples/mxf_video.mxf' ~/'Documents/Samples/SAMPLE WORD DOCUMENT.docx' --ts=@json:'{"target_rate_kbps":1000000}'
ascli -h
...
ascli aspera admin ats access_key create --cloud=aws --region=CF_AWS_REGION --params=@json:'{"id":"ak_aws","name":"laurent key AWS","storage":{"type":"aws_s3","bucket":"'CF_AWS_BUCKET'","credentials":{"access_key_id":"my_access_key_id_here","secret_access_key":"my_secret_access_key_here"},"path":"/"}}'
ascli aspera admin ats access_key create --cloud=softlayer --region=CF_ICOS_REGION --params=@json:'{"id":"akibmcloud","secret":"somesecret","name":"laurent key","storage":{"type":"ibm-s3","bucket":"CF_ICOS_BUCKET","credentials":{"access_key_id":"my_access_key_id_here","secret_access_key":"my_secret_access_key_here"},"path":"/"}}'
...
ascli aspera admin ats cluster show --cloud=aws --region=CF_AWS_REGION 
ascli aspera admin ats cluster show --id=1f412ae7-869a-445c-9c05-02ad16813be2
ascli aspera admin res client --id=CF_AOC1_CLIENT_ID set_pub_key @file:CF_PRIVATE_KEY_FILE
ascli aspera admin res node v3 events --secret=my_secret_here
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v3 access_key create --value=@json:'{"id":"testsub1","storage":{"path":"/folder1"}}'
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v3 access_key delete --id=testsub1
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v3 events
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v4 browse /
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v4 delete /folder1
ascli aspera admin resource node --name=CF_AOC_NODE1_NAME --secret=my_secret_here v4 mkdir /folder1
...
ascli aspera files http_node_download --to-folder=LOCAL_FOLDER /200KB.1
...
ascli aspera files upload --to-folder=/ CF_SAMPLE_FILEPATH
...
ascli aspera packages send --value=@json:'{"name":"'"CURRENT_DATE"'","recipients":["laurent.martin.aspera@fr.ibm.com"],"note":"my note"}' CF_SAMPLE_FILEPATH
ascli aspera packages send --value=@json:'{"name":"'"CURRENT_DATE"'","recipients":["laurent.martin.l+external@gmail.com"]}' --new-user-option=@json:'{"package_contact":true}' CF_SAMPLE_FILEPATH
ascli aspera packages send --workspace="CF_AOC_WS_SH_BX" --value=@json:'{"name":"'"CURRENT_DATE"'","recipients":["CF_AOC_SH_BX"]}' CF_SAMPLE_FILEPATH
...
ascli ats access_key create --cloud=aws --region=CF_AWS_REGION --params=@json:'{"id":"ak_aws","name":"laurent key AWS","storage":{"type":"aws_s3","bucket":"'CF_AWS_BUCKET'","credentials":{"access_key_id":"my_access_key_id_here","secret_access_key":"my_secret_access_key_here"},"path":"/"}}'
ascli ats access_key create --cloud=softlayer --region=CF_ICOS_REGION --params=@json:'{"id":"akibmcloud","secret":"somesecret","name":"laurent key","storage":{"type":"ibm-s3","bucket":"CF_ICOS_BUCKET","credentials":{"access_key_id":"my_access_key_id_here","secret_access_key":"my_secret_access_key_here"},"path":"/"}}'
...
ascli ats cluster show --cloud=aws --region=CF_AWS_REGION 
...
ascli config export
ascli config genkey LOCAL_FOLDER/mykey
ascli config id conf_name delete
ascli config id conf_name initialize @json:'{"p1":"v1","p2":"v2"}'
ascli config id conf_name set param value
ascli config id conf_name show
ascli config id conf_name update --p1=v1 --p2=v2
ascli config id default set shares conf_name
ascli config list
ascli config open
...
ascli cos node --service-credentials=@json:@file:CF_ICOS_CREDS_FILE --region=CF_ICOS_REGION --bucket=CF_ICOS_BUCKET access_key --id=self show
ascli cos node --service-credentials=@json:@file:CF_ICOS_CREDS_FILE --region=CF_ICOS_REGION --bucket=CF_ICOS_BUCKET download CF_SAMPLE_FILENAME
ascli cos node --service-credentials=@json:@file:CF_ICOS_CREDS_FILE --region=CF_ICOS_REGION --bucket=CF_ICOS_BUCKET info
ascli cos node --service-credentials=@json:@file:CF_ICOS_CREDS_FILE --region=CF_ICOS_REGION --bucket=CF_ICOS_BUCKET upload CF_SAMPLE_FILEPATH
...
ascli faspex package recv --link='CF_FASPEX_PUBLINK_RECV_PACKAGE'
ascli faspex package recv --to-folder=LOCAL_FOLDER --id=ALL --once-only=yes
ascli faspex package send --delivery-info=@json:'{"title":"'"CURRENT_DATE"'","recipients":["laurent.martin.aspera@fr.ibm.com"]}' CF_SAMPLE_FILEPATH
ascli faspex package send --link='CF_FASPEX_PUBLINK_SEND_DROPBOX' --delivery-info=@json:'{"title":"'"CURRENT_DATE"'"}' CF_SAMPLE_FILEPATH
ascli faspex package send --link='CF_FASPEX_PUBLINK_SEND_TO_USER' --delivery-info=@json:'{"title":"'"CURRENT_DATE"'"}' CF_SAMPLE_FILEPATH
...
ascli faspex5 package send --value=@json:'{"title":"test title","recipients":["admin"]}' CF_SAMPLE_FILEPATH
...
ascli node browse / -r
ascli node delete CF_HSTS_FOLDER_UPLOAD/CF_SAMPLE_FILENAME
ascli node download --to-folder=LOCAL_FOLDER CF_HSTS_FOLDER_UPLOAD/CF_SAMPLE_FILENAME
ascli node info
...
ascli node upload --to-folder=CF_HSTS_FOLDER_UPLOAD --ts=@json:'{"target_rate_cap_kbps":10000}' CF_SAMPLE_FILEPATH
ascli orchestrator info
ascli orchestrator plugins
ascli orchestrator processes
ascli orchestrator workflow --id=CF_ORCH_WORKFLOW_ID inputs
ascli orchestrator workflow --id=CF_ORCH_WORKFLOW_ID start --params=@json:'{"Param":"laurent"}'
ascli orchestrator workflow --id=CF_ORCH_WORKFLOW_ID start --params=@json:'{"Param":"laurent"}' --result=ResultStep:Complete_status_message
ascli orchestrator workflow --id=CF_ORCH_WORKFLOW_ID status
...
ascli preview test --case=$@ mp4 ~/'Documents/Samples/mxf_video.mxf' --video-conversion=blend --log-level=debug
ascli preview test --case=$@ mp4 ~/'Documents/Samples/mxf_video.mxf' --video-conversion=clips --log-level=debug
ascli preview test --case=$@ mp4 ~/'Documents/Samples/mxf_video.mxf' --video-conversion=reencode --log-level=debug
ascli preview test --case=$@ png ~/'Documents/Samples/SAMPLE WORD DOCUMENT.docx' --log-level=debug
ascli preview test --case=$@ png ~/'Documents/Samples/YıçşöğüİÇŞÖĞÜ.pdf' --log-level=debug
ascli preview test --case=$@ png ~/'Documents/Samples/mxf_video.mxf' --video-png-conv=animated --log-level=debug
ascli preview test --case=$@ png ~/'Documents/Samples/mxf_video.mxf' --video-png-conv=fixed --log-level=debug
ascli preview test --case=$@ png ~/Documents/Samples/anatomic-2k/TG18-CH/TG18-CH-2k-01.dcm --log-level=debug
ascli preview trevents --once-only=yes --skip-types=office --log-level=info
ascli server browse /
ascli server browse CF_HSTS_FOLDER_UPLOAD/target_hot
ascli server cp NEW_SERVER_FOLDER/CF_SAMPLE_FILENAME CF_HSTS_FOLDER_UPLOAD/200KB.2
ascli server delete CF_HSTS_FOLDER_UPLOAD/target_hot
ascli server delete CF_HSTS_FOLDER_UPLOAD/to.delete
ascli server delete NEW_SERVER_FOLDER
ascli server df
ascli server download NEW_SERVER_FOLDER/CF_SAMPLE_FILENAME --to-folder=CF_HSTS_FOLDER_UPLOAD --transfer=node
ascli server download NEW_SERVER_FOLDER/CF_SAMPLE_FILENAME --to-folder=LOCAL_FOLDER
ascli server du /
ascli server info
ascli server md5sum NEW_SERVER_FOLDER/CF_SAMPLE_FILENAME
ascli server mkdir CF_HSTS_FOLDER_UPLOAD/target_hot
ascli server mkdir NEW_SERVER_FOLDER --logger=stdout
ascli server mv CF_HSTS_FOLDER_UPLOAD/200KB.2 CF_HSTS_FOLDER_UPLOAD/to.delete
ascli server upload --sources=@ts --ts=@json:'{"paths":[{"source":"CF_SAMPLE_FILEPATH","destination":"NEW_SERVER_FOLDER/othername"}]}'
ascli server upload --src-type=pair --sources=@json:'["CF_SAMPLE_FILEPATH","NEW_SERVER_FOLDER/othername"]'
ascli server upload --src-type=pair CF_SAMPLE_FILEPATH NEW_SERVER_FOLDER/othername
ascli server upload --to-folder=CF_HSTS_FOLDER_UPLOAD/target_hot --lock-port=12345 --ts=@json:'{"EX_ascp_args":["--remove-after-transfer","--remove-empty-directories","--exclude-newer-than=-8","--src-base","source_hot"]}' source_hot
ascli server upload CF_SAMPLE_FILEPATH --to-folder=NEW_SERVER_FOLDER
ascli shares repository browse /
ascli shares repository delete /CF_SHARES_UPLOAD/CF_SAMPLE_FILENAME
ascli shares repository download --to-folder=LOCAL_FOLDER /CF_SHARES_UPLOAD/CF_SAMPLE_FILENAME
ascli shares repository upload --to-folder=/CF_SHARES_UPLOAD CF_SAMPLE_FILEPATH
...
ascli sync start --parameters=@json:'{"sessions":[{"name":"test","reset":true,"remote_dir":"/sync_test","local_dir":"contents","host":"CF_HSTS_ADDR","user":"user1","private_key_path":"CF_HSTS_TEST_KEY"}]}'
...
$ ascli -h
NAME
	ascli -- a command line tool for Aspera Applications (v1.0.0)
...
DESCRIPTION
	...
	Documentation and examples: https://rubygems.org/gems/aspera
	execute: ascli conf doc
	or visit: http://www.rubydoc.info/gems/aspera
